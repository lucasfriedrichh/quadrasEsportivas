...
BEGIN
    ...
    IF valor_medio > 0 THEN
        SELECT CONCAT('O valor médio das reservas realizadas pelo cliente ', nome, ' é de R$', valor_medio) AS resultado FROM cliente WHERE id = id_cliente;
    ELSE
        SELECT CONCAT('O cliente ', nome, ' ainda não realizou nenhuma reserva.') AS resultado FROM cliente WHERE id = id_cliente;
    END IF;
END;
